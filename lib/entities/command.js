...
module.exports = class AkashiCommand {
    ...
    help() {
        const embed = this
            ...
            .addField('Category', this.client.util.uppercase(this.category), true)
            .addField('Aliases', this.aliases.length > 1? this.aliases.map(s => `\`${s}\``).join(', '): 'No aliases', true)
            .addField('Guild Only', this.guildOnly? 'Yes': 'No', true)
            .addField('Owner Only', this.ownerOnly? 'Yes': 'No', true);

        if (this.subcommands.length > 1) embed.addField('Subcommands', this.subcommands.map(sub => stripIndents`
            **${sub.name}**: \`${typeof sub.description === 'function'? sub.description(client): sub.description}\`
        `).join('\n'), false);
 ...
 * @prop {string|((client: import('../client')) => string)} description The command's description as an string or an function to return a string
 ...
 * @prop {string|((client: import('../client')) => string)} description The subcommand's description
 * @prop {(client: import('../client'), ctx: import('./context')) => Promise<void>} run The run function to run the subcommand
 */